 ...
 *
 * <b>담는 것은 근거 검사와 두 읽기 합치기까지 마친 결과입니다.</b>
 * 두 번 읽을 때 한쪽 답만 담으면 다시 쓸 때 합치기를 또 해야 하고, 합친 결과가 같은 문구마다 같아야
 * 한 실행 안의 같은 공지가 같은 조건을 받습니다.
 *
...
public class LlmReuse {
    ...
    private final Map<List<Segment>, LlmReading> readings = new HashMap<>();
    private int calls;
    private int reused;

    public LlmReading readingFor(List<Segment> segments, Supplier<LlmReading> call) {
        LlmReading known = readings.get(segments);
        if (known != null) {
            reused++;
            return known;
        }
        calls++;
        LlmReading reading = call.get();
        readings.put(List.copyOf(segments), reading);
        return reading;
    }
     ...
     * 두 번 읽으면 입력 하나에 API 호출이 둘이지만 여기서는 한 번입니다 — 새로 읽은 입력의 수입니다.
