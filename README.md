...
⚠**policy 는 v0.1.1 이상이어야 합니다.** 그 전 판은 범위 값 "동반 불가"(`NONE`)를 모르는데, 이 서비스는
동반 불가 원문을 그 값으로 보냅니다. 판이 낮으면 첫 청크의 `bulk` 가 400 으로 돌아옵니다.
...
| 범위가 비어 있는데 허용 구역이나 제외 구역이 있거나, 실내 `false` · 실외 `true` | 범위를 일부 구역(`PARTIAL`)으로 |
...
| 채운 칸에 원래 칸의 근거를 옮겨 씀 | 판정 화면이 칸마다 근거를 보여 주므로, 근거가 없으면 그 자리가 비어 보임 |
...
| `items[].evidence[]` | 근거 — `fieldName` · `originField` · `segmentIndex` · `segmentText` |
| `items[].conflicts[]` | 소스 내 충돌 — `fieldName` · `sourceValues` 는 정확히 `field` · `text` 두 키 |
| `items[].extractionMethod` | `RULE` · `LLM` · `MIXED` |
...
| 본문에 보이는 것 | 까닭 | 할 일 |
|---|---|---|
| 범위 값 `NONE` 을 읽지 못함 | policy 가 v0.1.1 보다 낮음 | policy 를 올림 |
